...
function ReportView({ report }) {
  ...
  const upside = financial.current_price && r.target_price
    ? ((r.target_price - financial.current_price) / financial.current_price * 100).toFixed(1)
    : null
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-6">
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-3xl font-bold text-slate-900">
              {report.company_name} <span className="text-slate-500 font-normal">({report.ticker})</span>
            </h1>
            <p className="text-slate-500 mt-1">{financial.sector} — {financial.industry}</p>
          </div>
        ...
        <div className={`mt-6 p-6 border-l-4 rounded ${recClass}`}>
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div>
              <p className="text-sm uppercase tracking-wide opacity-75">Recommendation</p>
              <p className="text-4xl font-bold mt-1">{r.recommendation}</p>
            </div>
            <div className="grid grid-cols-3 gap-6 text-right">
              <div>
                <p className="text-xs uppercase opacity-75">Target</p>
                <p className="text-xl font-semibold">${r.target_price?.toFixed(2) || 'N/A'}</p>
              </div>
              <div>
                <p className="text-xs uppercase opacity-75">Upside</p>
                <p className={`text-xl font-semibold ${upside >= 0 ? 'text-green-700' : 'text-red-700'}`}>
                  {upside !== null ? `${upside > 0 ? '+' : ''}${upside}%` : 'N/A'}
                </p>
              </div>
              <div>
                <p className="text-xs uppercase opacity-75">Confidence</p>
                <p className="text-xl font-semibold">{r.confidence?.toFixed(0)}%</p>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* Executive Summary */}
        <div className="mt-6">
          <h3 className="text-sm uppercase tracking-wide text-slate-500 mb-2">Executive Summary</h3>
          <p className="text-slate-700 leading-relaxed">{r.executive_summary}</p>
        </div>
        ...
        <div className="mt-6 grid md:grid-cols-2 gap-4">
          ...
          <div className="p-4 bg-red-50 border-l-4 border-red-400 rounded">
            ...
            <ul className="space-y-1 text-sm text-red-900">
              {r.key_risks?.map((risk, i) => <li key={i}>• {risk}</li>)}
            </ul>
          </div>
        </div>
      </div>
      ...
      <Section title="Technical Analysis">
        <div className="grid md:grid-cols-2 gap-4 mb-4">
          <MetricCard title="Indicators" items={[
          ...
          <MetricCard title="Key Levels" items={[
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              <tr className="bg-amber-50 font-semibold border-b border-slate-200">
                ...
                <td className="p-3 text-right">{formatNum(financial.market_cap)}</td>
                <td className="p-3 text-right">{financial.pe_ratio?.toFixed(2)}</td>
                <td className="p-3 text-right">{formatPct(financial.revenue_growth)}</td>
                <td className="p-3 text-right">{formatPct(financial.profit_margin)}</td>
              </tr>
              {competitor.competitors?.map((c) => (
                <tr key={c.ticker} className="border-b border-slate-100">
                  ...
                  <td className="p-3 text-right">{formatPct(c.revenue_growth)}</td>
                  <td className="p-3 text-right">{formatPct(c.profit_margin)}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <Section title="News Sentiment">
        <div className="mb-4 flex items-center gap-4 flex-wrap">
          ...
          <span className={`px-3 py-1 rounded-full text-sm font-medium ${
            news.overall_sentiment === 'Positive' ? 'bg-green-100 text-green-800' :
            news.overall_sentiment === 'Negative' ? 'bg-red-100 text-red-800' :
            'bg-slate-100 text-slate-800'
          }`}>
            {news.overall_sentiment}
          </span>
          <span className="text-sm text-slate-600">
            {news.positive_count} positive • {news.negative_count} negative • {news.neutral_count} neutral
          </span>
        </div>
        <p className="text-slate-700 mb-4">{news.analysis_summary}</p>
        <div className="space-y-2">
          {news.articles?.slice(0, 10).map((article, i) => (
            <div key={i} className={`p-3 border-l-4 rounded text-sm ${sentimentColors[article.sentiment] || sentimentColors['Neutral']}`}>
      ...
      <div className="text-center text-xs text-slate-400 pt-4">
        Analysis completed in {report.total_time_seconds}s • Generated by finAG AI
      </div>
    </div>
  )
}
